...
func correctTime(srcFile, srcTime string, isEnd bool) (string, error) {
	if isEnd && srcTime == "0" {
		...
		return subtractSecondsFromTime(endTime, 10)
	}
...
func main() {
	...
	// inputFile := "ESL0002 – Getting Up.mp3"
	// outputDir := "output_chunks" // 输出目录

	// 示例1: 按固定时长切割（每30秒一个片段）
	// if err := SplitAudioByDuration(inputFile, outputDir, 30); err != nil {
	// 	fmt.Printf("切割失败: %v\n", err)
	// 	return
	// }

	// lyrics, err := ReadLyrics(inputFile)
	// if err != nil {
	// 	panic(err)
	// }
	// fmt.Println("lyrics:", lyrics)

	// 示例2: 按指定时间段切割（从00:01:20到00:03:15）

	// ext := filepath.Ext(inputFile)
	// baseName := strings.TrimSuffix(filepath.Base(inputFile), ext)
	// outputFile := baseName + "-s.mp3"
	// if err := SplitAudioByTime(inputFile, outputFile, "00:01:20", "00:03:15"); err != nil {
	// 	fmt.Printf("切割失败: %v\n", err)
	// 	return
	// }
